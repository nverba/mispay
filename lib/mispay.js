...
function MisPay(_options) {
	...
	function _makeRequest(method, path, data, cbok, cberror){
		...
		console.log( '_makeRequest',  method, path, data);
	...
	var _post = function(path, data, success, failure){
		console.log('_post custom', path, data, success, failure);
	...
	var _cardman = {
		...
		update: function(pid, cid, cb, fail){
			console.log('_cardman payin update', 'payin/' + pid + '/card');
			_post('payin/' + pid + '/card', {cid:cid}, cb, fail);
		},
		disable: function(uid, cardid, cb, fail){
			_put('buyers/'+uid+'/disable/card', {cid:cardid}, cb, fail);
		},
		...
		repreauth: function(pid, d, cb, fail){
			_put('payin/' + pid + '/preauth', d, cb, fail);
		},
		repay: function(pid, d, cb, fail){
			_put('payin/' + pid + '/pay', d, cb, fail);
		},
	};
	...
	function mango_callback(req, res, next){
		for(var k in req.query){
			...
			res.send("<script>console.log('from mangopay', parent)</script>"); // "<script>console.log('from mangopay', parent)</script>"
			break;
		}
	}
	...
	var _route = function(router){
		router.get('/mispay/islive', function(req, res, next){
			res.send({live: _live ? true : false});
		});
		...
		router.post('/mispay/payin/:pid/card', function(req, res, next){ console.log('/mispay/payin/:pid/card', req.params.pid, req.body.cid);
			_cardman.update(req.params.pid, req.body.cid, forward(res), next);
		});
		router.put('/mispay/buyer/:uid/card/:cid/disable', function(req, res, next){
			_cardman.disable(req.params.uid, req.params.cid, forward(res), next);
		});
		router.post('/mispay/payin/:pid/pay', function(req, res, next){
			_cardman.pay(req.params.pid, req.body, doTransfer(req.params.pid, res), next);
		});
		router.put('/mispay/payin/:pid/pay', function(req, res, next){
			_cardman.repay(req.params.pid, req.body, doTransfer(req.params.pid, res), next);
		});
		router.post('/mispay/payin/:pid/preauth', function(req, res, next){
			_cardman.preauth(req.params.pid, req.body, forward(res), next);
		});
		router.put('/mispay/payin/:pid/preauth', function(req, res, next){
			_cardman.repreauth(req.params.pid, req.body, doTransfer(req.params.pid, res), next);
		});
		router.post('/mispay/payin/:pid/prepay', function(req, res, next){
			...
			// 	}				
			// }, next);
			_cardman.prepay(req.params.pid, req.body, doTransfer(req.params.pid, res), next);
		});
	...
	/**
	 * Once a pay, repay is successful, do a transfer immediately.
	 */
	function doTransfer(pid, res) {
		return function(r){
			if (_auto_transfer && r.Status && r.Status=='SUCCEEDED'){
				_distribute(pid, function(r2){
					r.Transferred = r2;
					if (r.DebitedFunds){
						var hd = {currency:r.DebitedFunds.Currency, amount:r.DebitedFunds.Amount};
						r.hash = makeHash(hd, _gapi_hashkey);
