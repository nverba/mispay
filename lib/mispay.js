...
function MisPay(_username, _password, _options) {
	...
	var _regcard = function(payinid, cb, fail){
		_get('payin/' + payinid + '/regcard', cb, fail);
	};
	...
	var _cardman = {
		...
		distribute: function(pid, cb, fail){
	...
	var _route = function(router){
		router.post('/mispay/request', function(req, res, next){
			_request(req.body, res.send, next);
		});
		router.get('/mispay/checkout', function(req, res, next){
			_checkout(req.body.rid, res.send, next);
		});
		router.get('/mispay/payin/:pid/regcard', function(req, res, next){
			_cardman.register(req.params.pid, res.send, next);
		});
		router.post('/mispay/payin/:pid/card', function(req, res, next){
			_cardman.update(req.params.pid, req.body.cid, res.send, next);
		});
		router.post('/mispay/payin/:pid/pay', function(req, res, next){
			_cardman.pay(req.params.pid, req.body, function(r){
				if (_auto_transfer && r.Status && r.Status=='SUCCEEDED'){
					_cardman.distribute(req.params.pid, function(r2){
						r.Transferred = r2;
						res.send(r);
					}, function(e){
						res.send(r);
		...
		router.post('/mispay/payin/:pid/preauth', function(req, res, next){
			_cardman.preauth(req.params.pid, req.body, res.send, next);
		});
		...
		router.post('/mispay/payin/:pid/transfer', function(req, res, next){
			_cardman.distribute(req.params.pid, {}, res.send, next);
		});
		router.get('/mispay/mango/callback', function(req, res, next){
