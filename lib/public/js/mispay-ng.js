...
function MisPayCtr($scope, mangoPay, misPay) {
	...
	$scope.Payment = {
		...
		request: function(basket, cb){
			...
			misPay.Request.post(basket, function(rd){
				if (rd.id) {
					requestId = rd.id;
					setTimeout(function(){checkout(cb);}, 200);
					//if(cb) cb(null, rd);
				} else {
					if (cb) cb(rd);
				}
		...
		pay: function(pid, cb){
			...
			misPay.Payment.pay(pid, {cid:p.usecard, ReturnURL:MANGO_CB_URL}, function(rd){
				if (rd.Status=='SUCCEEDED'){
					...
					//$scope.secureRedirect(rd.SecureModeRedirectURL, pid);
				} else {
					console.log(rd);
					if(cb) cb(rd);
				}
		...
		preauth: function(pid, cb){
			...
			misPay.Payment.preauth(pid, {cid:p.usecard,ReturnURL:MANGO_CB_URL}, function(rd){
				if (rd.Status=='SUCCEEDED'){
					if(cb)cb(null,rd);
					//$scope.succeeded('preauth',rd);
				} else if (rd.Status=='CREATED' && rd.SecureModeNeeded){
					//$scope.secureRedirect(rd.SecureModeRedirectURL, pid);
				} else {
					if(cb)cb(rd);
					//$scope.failed('preauth',rd);
				}
			});
		},
		prepay: function(pid, cb){
			...
			misPay.Payment.prepay(pid, d, function(rd){
				if (rd.Status=='SUCCEEDED'){
					//$scope.succeeded('prepay',rd);
				} else {
					//$scope.failed('prepay',rd);
				}
			});
		},
	};
	...
	$scope.Card = {
		...
		register: function(card, cb){
			if (!payinId || !cardReg){
				console.log('No pid or card not pre-registered');
				if(cb) cb('preRegister not called');
				return;
			}
			
			if (!card) card = $scope.card;
			if (!card || !card.cardNumber){
				console.log('No proper $scope.card found');
				return;
			}
